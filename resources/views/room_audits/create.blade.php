<x-app-layout>
    <x-slot name="header">New Room Audit</x-slot>

    <div class="max-w-lg mx-auto bg-gray-800 rounded-lg p-6">
        <form action="{{ route('room_audits.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-4">
                ...
                <input type="date" name="audit_date" id="audit_date" value="{{ old('audit_date') }}" class="w-full px-3 py-2 rounded bg-gray-700 text-white">
            </div>

            <div class="mb-4">
                <label for="notes" class="block text-gray-200 mb-1">Notes</label>
                <textarea name="notes" id="notes" rows="3" class="w-full px-3 py-2 rounded bg-gray-700 text-white">{{ old('notes') }}</textarea>
            </div>

            <div class="flex justify-end space-x-2">
                <a href="{{ route('room_audits.index') }}" class="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded">Cancel</a>
                <button type="submit" class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded">Save Audit</button>
            </div>
        </form>
    </div>
</x-app-layout>