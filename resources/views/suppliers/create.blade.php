<x-app-layout>
    ...
    <form action="{{ route('suppliers.store') }}" method="POST" class="bg-gray-800 p-6 rounded shadow text-white max-w-xl mx-auto">
        @csrf

        <div class="mb-4">
            <label for="name" class="block mb-1">Supplier Name</label>
            <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full p-2 bg-gray-700 rounded">
            @error('name') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
        </div>

        <div class="mb-4">
            <label for="contact_info" class="block mb-1">Contact Info</label>
            <input type="text" name="contact_info" id="contact_info" value="{{ old('contact_info') }}" class="w-full p-2 bg-gray-700 rounded">
